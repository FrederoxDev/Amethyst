    ...
    public:
        ...
        */
        ...
        [[nodiscard]] E GetNextValue() {
            ...
            Assert(it != mEnumsToCurrent.end(), "Tried to allocate a value for '{}', however the enum has not been registered yet.", typeid(E).name());

            // Check the underlying storage type and see if there is still room.
            uint64_t mUnderlyingLimit = static_cast<uint64_t>(std::numeric_limits<E>::max());

            // Well... fuck
            Assert(it->second != mUnderlyingLimit, "Tried to allocate another value for enum '{}', however the maximum limit for the underlying type", typeid(E).name());
