...
bool ProtectMemory(uintptr_t address, size_t size, DWORD protectionData, DWORD* oldProtection)
{
    DWORD oldProtect;
    ...
    if (!VirtualProtect(reinterpret_cast<void*>(address), size, protectionData, &oldProtect)) {
		Log::Error("Failed to reprotect memory at 0x{:X}! Error: {}", address, GetLastError());
		return false;
	}
