...
//    uintptr_t size = GetMinecraftSize() - std::strlen(mask);
//
...
DWORD WINAPI Main() {
    ...
    Log::Info("Zenova_Sigscan 10 total time: {}ms\n", duration.count());
    Log::Info("  Average time: {}ms\n\n", (float)duration.count() / 10);*/
    ...
    Log::Info("Freddie_Sigscan ran 200 total time: {}ms\n", duration.count());
    Log::Info("  Average time: {}ms\n\n", (float)duration.count() / 200);
