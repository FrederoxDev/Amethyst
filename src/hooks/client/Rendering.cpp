...
namespace Amethyst::ClientHooks::RenderingHooks {
	...
	void LevelRenderer_renderLevel(LevelRenderer* self, ScreenContext& screenContext, const FrameRenderObject& frameRenderObj) {
		Amethyst::EventBus& bus = Amethyst::GetEventBus();
		BeforeRenderLevelEvent beforeEvent(*self, screenContext, const_cast<FrameRenderObject&>(frameRenderObj));
		bus.Invoke(beforeEvent);
		_LevelRenderer_renderLevel(self, screenContext, frameRenderObj);
		AfterRenderLevelEvent afterEvent(*self, screenContext, const_cast<FrameRenderObject&>(frameRenderObj));
