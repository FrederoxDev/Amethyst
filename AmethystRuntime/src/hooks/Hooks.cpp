...
void CreateModFunctionHooks() {
    ...
    if (runtime->mModRender.size() > 0) hookManager->CreateHook(
    ...
    if (runtime->mModStartJoinGame.size() > 0) hookManager->CreateHook(
    ...
    if (runtime->mModTickAfter.size() > 0 || runtime->mModTickBefore.size() > 0) hookManager->CreateHook(
